...
// ============================================================
// 1. FUNGSI HELPER: Menentukan Style Berdasarkan Jenis Donasi
// ============================================================
function getDonationStyle(jenis) {
    let style = {
        bgIcon: '',
        iconClass: '',
        textTypeClass: '',
    ...
    switch (jenis) {
        case 'Zakat Fitrah':
            style.bgIcon = 'bg-emerald-100 text-emerald-600';
            style.iconClass = 'fa-bowl-rice';
            style.textTypeClass = 'text-emerald-600';
            style.borderTypeClass = 'border-emerald-100';
            style.borderLeftClass = 'bg-emerald-500';
            break;

        case 'Zakat Maal':
            style.bgIcon = 'bg-amber-100 text-amber-600';
            style.iconClass = 'fa-sack-dollar';
            style.textTypeClass = 'text-amber-600';
            style.borderTypeClass = 'border-amber-100';
            style.borderLeftClass = 'bg-amber-500';
            break;

        case 'Infaq Pengembangan Kampus':
            style.bgIcon = 'bg-rose-100 text-rose-600';
            style.iconClass = 'fa-school';
            style.textTypeClass = 'text-rose-600';
            style.borderTypeClass = 'border-rose-100';
            style.borderLeftClass = 'bg-rose-500';
            break;

        case 'Infaq Beasiswa Pendidikan':
            style.bgIcon = 'bg-sky-100 text-sky-600';
            style.iconClass = 'fa-user-graduate';
            style.textTypeClass = 'text-sky-600';
            style.borderTypeClass = 'border-sky-100';
            style.borderLeftClass = 'bg-sky-500';
            break;

        case 'Infaq Umum':
            style.bgIcon = 'bg-violet-100 text-violet-600';
            style.iconClass = 'fa-parachute-box';
            style.textTypeClass = 'text-violet-600';
            style.borderTypeClass = 'border-violet-100';
            style.borderLeftClass = 'bg-violet-500';
            break;

        default: // Default Infaq / Lainnya
            style.bgIcon = 'bg-orange-100 text-orange-600';
            style.iconClass = 'fa-hand-holding-heart';
            style.textTypeClass = 'text-orange-600';
            style.borderTypeClass = 'border-orange-100';
            style.borderLeftClass = 'bg-orange-500';
            break;
    }
    return style;
}
...
// ============================================================
// 2. FUNGSI RENDER: Menampilkan Data ke HTML
// ============================================================
function renderRiwayatCard(item) {
    // Ambil style berdasarkan jenis donasi
    const s = getDonationStyle(item.JenisDonasi);
    
    // Format Rupiah
    const nominalFormatted = parseInt(item.Nominal).toLocaleString('id-ID', { 
        style: 'currency', 
        currency: 'IDR', 
        minimumFractionDigits: 0 
    });

    // Format Waktu (Relative Time) - Contoh Sederhana
    const dateObj = new Date(item.Timestamp);
    const dateStr = dateObj.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' });
    const timeStr = dateObj.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
    
    // Cek Alumni Badge
    const alumniBadge = item.IsAlumni ? 
        `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-slate-800 text-white" title="Alumni Mu'allimin"><i class="fas fa-graduation-cap"></i> Alumni</span>` : '';

    // Pesan (Quote)
    const pesanHtml = item.Pesan ? `
        <div class="mt-3 bg-slate-50 rounded-xl p-3 text-xs text-slate-500 italic leading-relaxed border border-slate-100 relative">
            <i class="fas fa-quote-left text-slate-200 absolute top-2 left-2 -z-10 text-xl"></i>
            "${item.Pesan}"
        </div>` : '';
    ...
    <div class="group relative bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 transition-all duration-300 h-full overflow-hidden">
        ...
        <div class="absolute left-0 top-0 bottom-0 w-1 ${s.borderLeftClass} opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
        ...
        <div class="relative z-10 flex flex-col sm:flex-row gap-5 items-start">
            ...
            <div class="w-14 h-14 rounded-2xl ${s.bgIcon} flex items-center justify-center text-2xl shadow-sm shrink-0 group-hover:scale-110 group-hover:-rotate-6 transition-transform duration-300">
                <i class="fas ${s.iconClass}"></i>
            </div>
            ...
            <div class="flex-1 w-full min-w-0">
                <div class="flex flex-col sm:flex-row justify-between items-start gap-2">
                    ...
                    <div>
                        <div class="flex items-center flex-wrap gap-2 mb-1">
                            <h4 class="font-bold text-slate-800 text-lg leading-tight line-clamp-1 group-hover:text-slate-900" title="${item.NamaDonatur || 'Hamba Allah'}">
                                ${item.NamaDonatur || 'Hamba Allah'}
                            </h4>
                            ${alumniBadge}
                        </div>
                        ...
                        <div class="flex items-center gap-2 text-xs flex-wrap">
                            <span class="font-bold ${s.textTypeClass} uppercase tracking-wide bg-white px-2 py-0.5 rounded border ${s.borderTypeClass}">
                                ${item.JenisDonasi}
                            </span>
                            <span class="text-slate-300 hidden sm:inline">•</span>
                            <span class="flex items-center gap-1 text-slate-500 font-medium">
                                <i class="fas fa-wallet text-[10px] opacity-50"></i> ${item.MetodePembayaran}
                            </span>
                        </div>
                    </div>
...
dataDonasi.forEach(item => {
    // Panggil fungsi renderRiwayatCard yang baru kita buat
    htmlContent += renderRiwayatCard(item); 
});

container.innerHTML = htmlContent;

function renderHomeCard(item) {
    // 1. TENTUKAN STYLE BERDASARKAN JENIS DONASI
    let s = {
    ...
    switch (item.JenisDonasi) {
        case 'Zakat Fitrah':
            s.icon = 'fa-bowl-rice';
            s.bgIcon = 'bg-emerald-100 text-emerald-600';
            s.badge = 'bg-emerald-50 text-emerald-600 border-emerald-100';
            s.hoverBorder = 'group-hover:border-emerald-200';
            s.hoverBg = 'group-hover:bg-emerald-50/30';
            s.hoverText = 'group-hover:text-emerald-600';
            s.clockColor = 'text-emerald-400';
            break;
        case 'Zakat Maal':
            s.icon = 'fa-sack-dollar';
            s.bgIcon = 'bg-amber-100 text-amber-600';
            s.badge = 'bg-amber-50 text-amber-600 border-amber-100';
            s.hoverBorder = 'group-hover:border-amber-200';
            s.hoverBg = 'group-hover:bg-amber-50/30';
            s.hoverText = 'group-hover:text-amber-600';
            s.clockColor = 'text-amber-400';
            break;
        case 'Infaq Pengembangan Kampus':
            s.icon = 'fa-school';
            s.bgIcon = 'bg-rose-100 text-rose-600';
            s.badge = 'bg-rose-50 text-rose-600 border-rose-100';
            s.hoverBorder = 'group-hover:border-rose-200';
            s.hoverBg = 'group-hover:bg-rose-50/30';
            s.hoverText = 'group-hover:text-rose-600';
            s.clockColor = 'text-rose-400';
            break;
        case 'Infaq Beasiswa Pendidikan':
            s.icon = 'fa-user-graduate';
            s.bgIcon = 'bg-sky-100 text-sky-600';
            s.badge = 'bg-sky-50 text-sky-600 border-sky-100';
            s.hoverBorder = 'group-hover:border-sky-200';
            s.hoverBg = 'group-hover:bg-sky-50/30';
            s.hoverText = 'group-hover:text-sky-600';
            s.clockColor = 'text-sky-400';
            break;
        case 'Infaq Umum':
            s.icon = 'fa-parachute-box';
            s.bgIcon = 'bg-violet-100 text-violet-600';
            s.badge = 'bg-violet-50 text-violet-600 border-violet-100';
            s.hoverBorder = 'group-hover:border-violet-200';
            s.hoverBg = 'group-hover:bg-violet-50/30';
            s.hoverText = 'group-hover:text-violet-600';
            s.clockColor = 'text-violet-400';
            break;
    }
    ...
    // Pastikan fungsi timeAgo() sudah ada di script Anda, atau gunakan item.Timestamp langsung
    const waktu = typeof timeAgo === 'function' ? timeAgo(item.Timestamp) : item.Timestamp;

    // 2. RETURN TEMPLATE HTML
    return `
    <div class="relative bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 border border-slate-100 transition-all duration-300 group hover:-translate-y-1 h-full flex flex-col justify-between overflow-hidden">
        
        <div class="absolute -right-4 -top-4 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity duration-500 rotate-12">
            <i class="fas ${s.icon} text-9xl text-slate-800"></i>
        </div>

        <div class="relative z-10">
            <div class="flex items-start justify-between mb-4">
                <div class="w-12 h-12 rounded-xl ${s.bgIcon} flex items-center justify-center text-lg shadow-sm ring-4 ring-white group-hover:scale-110 transition-transform duration-300">
                    <i class="fas ${s.icon}"></i>
                </div>
                
                <span class="text-[10px] font-bold ${s.badge} border px-2.5 py-1 rounded-lg uppercase tracking-wider shadow-sm">
                    ${item.JenisDonasi}
                </span>
            </div>
            ...
            <div>
                ...
                <div class="bg-slate-50 rounded-xl p-3 border border-slate-100 ${s.hoverBorder} ${s.hoverBg} transition-colors duration-300">
                    <div class="flex items-baseline gap-1">
                        <span class="text-xs text-slate-500 font-medium">Rp</span>
                        <span class="text-xl md:text-2xl font-black text-slate-800 ${s.hoverText} transition-colors">
                            ${nominal}
                        </span>
                    </div>
                </div>
        ...
        <div class="relative z-10 mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-400">
            <div class="flex items-center gap-1.5">
                <i class="far fa-clock ${s.clockColor}"></i>
                <span>${waktu}</span>
            </div>
